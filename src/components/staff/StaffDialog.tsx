...
export function StaffDialog({ open, onOpenChange, staff, onSave, existingSkills = [], existingSoftwares = [] }: StaffDialogProps) {
  ...
  const handleSubmit = () => {
    ...
    onSave({ full_name: fullName.trim(), email, role_title: roleTitle, skills, softwares });
    onOpenChange(false);
  };
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          ...
          <DialogDescription>
            {staff ? 'Update staff member details.' : 'Add a new team member to your staff directory.'}
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div className="space-y-2">
              <Label>Role</Label>
              <Input value={roleTitle} onChange={e => setRoleTitle(e.target.value)} placeholder="3D Artist" />
            </div>
          </div>
            ...
            <div className="flex flex-wrap gap-1.5 mb-2">
              {skills.map(s => (
                <Badge key={s} variant="secondary" className="gap-1 text-xs">
                  {s}
                  <button onClick={() => removeTag(s, skills, setSkills)} className="hover:text-destructive">
                    <X className="w-3 h-3" />
                  </button>
                </Badge>
              ))}
            ...
            <div className="flex flex-wrap gap-1.5 mb-2">
              {softwares.map(s => (
                <Badge key={s} variant="outline" className="gap-1 text-xs">
                  {s}
                  <button onClick={() => removeTag(s, softwares, setSoftwares)} className="hover:text-destructive">
                    <X className="w-3 h-3" />
                  </button>
                </Badge>
              ))}
            ...
            <div className="relative">
              <Input
                ...
                onKeyDown={e => {
                  if (e.key === 'Enter' || e.key === ',') {
                    e.preventDefault();
                    addTag(softwareInput, softwares, setSoftwares);
                    setSoftwareInput('');
                  }
                }}
                placeholder="Type and press Enter..."
                ...
                <div className="absolute z-10 mt-1 w-full bg-popover border border-border rounded-md shadow-md">
                  {filteredSoftwareSuggestions.map(s => (
                    <button
                      key={s}
                      className="w-full text-left px-3 py-1.5 text-sm hover:bg-accent"
                      onClick={() => { addTag(s, softwares, setSoftwares); setSoftwareInput(''); }}
                    >
                      {s}
                    </button>
                  ))}
